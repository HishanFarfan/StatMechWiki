...
Como concepto fundacional, **Temperatura estadistica** fija parte del lenguaje que permite pasar de microestados a macroestados. En este contexto, decir que definicion de temperatura a partir de la entropia equivale a indicar que informacion microscopica se conserva y cual se resume estadisticamente.
...
Esta seccion iguala la extension de la entrada con el resto de la wiki. No introduce una definicion nueva de **Temperatura estadistica**; agrega pruebas de uso, relaciones matematicas y criterios de lectura para que todas las paginas tengan una profundidad comparable.

### Desarrollo extendido 1

En esta ampliacion, **Temperatura estadistica** se vuelve a leer desde una perspectiva de fundamentos. El objetivo es hacer explicitas las piezas que normalmente quedan comprimidas en una sola ecuacion: espacio de estados, restricciones, pesos estadisticos, normalizacion, observables, fluctuaciones y limites de validez. Esta lectura es util para estudiar, pero tambien para editar la wiki con criterio: cada afirmacion debe poder conectarse con una cantidad calculable.
